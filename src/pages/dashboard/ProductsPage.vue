...
async function handleSelectFiles(files: File[]) {
  pendingFiles.value = files.slice(0, 4);
  const temporaryUrls = pendingFiles.value.map((file) => URL.createObjectURL(file));
  form.imageUrls = [...form.imageUrls.filter((item) => item.startsWith('http')), ...temporaryUrls].slice(0, 4);
}
  ...
  <div class="d-flex flex-column ga-6">
    <v-row>
      <v-col cols="12" md="6" xl="3">
        <DashboardMetricCard label="Ativos" :value="stats.active" description="Itens publicados na vitrine." icon="mdi-check-circle-outline" color="success" />
      </v-col>
      <v-col cols="12" md="6" xl="3">
        <DashboardMetricCard label="Rascunhos" :value="stats.draft" description="Produtos ainda não publicados." icon="mdi-file-document-outline" color="warning" />
      </v-col>
      <v-col cols="12" md="6" xl="3">
        <DashboardMetricCard label="Baixo estoque" :value="stats.lowStock" description="Produtos com pouca disponibilidade." icon="mdi-alert-outline" color="error" />
      </v-col>
      <v-col cols="12" md="6" xl="3">
        <DashboardMetricCard label="Ticket médio" :value="stats.avgTicket" description="Preço médio do catálogo." icon="mdi-cash-multiple" color="primary" />
      </v-col>
    </v-row>
    ...
    <v-card class="glass-panel pa-5 pa-md-6">
      <div class="d-flex flex-column flex-lg-row justify-space-between align-start ga-4">
        <div>
          ...
          <div class="section-subtitle mt-1">A V3 já controla o volume de produtos conforme a camada SaaS preparada.</div>
        </div>
        <v-chip color="primary" variant="tonal">{{ currentPlan.name }} · {{ products.length }}/{{ productLimitLabel }}</v-chip>
      </div>
      <v-progress-linear :model-value="usagePercent" color="primary" height="10" rounded class="mt-4 mb-3" />
      <div class="text-body-2 text-medium-emphasis">
        {{ currentPlan.productLimit >= 999999 ? 'Seu plano atual não possui limitação prática.' : `${remainingProducts} vagas restantes para novos produtos.` }}
      </div>
    ...
    <AppSectionCard title="Catálogo de produtos" subtitle="Visualize o inventário com mais clareza, valide o limite do plano e edite rapidamente.">
      <template #actions>
        ...
        <v-table>
          ...
          <tbody>
            <tr v-for="product in visibleProducts" :key="product.id">
              <td>
                <div class="d-flex align-center ga-3 py-2">
                  <v-avatar rounded="lg" size="52">
                    <v-img :src="product.imageUrls[0] || 'https://placehold.co/800x800?text=Produto'" cover />
                  </v-avatar>
                  <div>
              ...
              <td>
                <v-chip size="small" :color="product.status === 'active' ? 'success' : 'warning'" variant="tonal">
                  {{ product.status }}
                </v-chip>
              </td>
              <td class="text-right">
                <div class="d-flex justify-end ga-2">
                  ...
                  <v-btn size="small" variant="text" color="error" @click="removeProduct(product.id)">Excluir</v-btn>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      >
        <div class="mt-4">
          <v-btn color="primary" :disabled="!canCreateProduct" @click="openCreate">Cadastrar primeiro produto</v-btn>
        </div>
      </EmptyState>
    </AppSectionCard>
  </div>
  ...
  <v-navigation-drawer v-model="drawer" location="right" temporary width="560">
    <div class="pa-5 d-flex flex-column ga-4">
      <div>
        ...
        <div class="text-body-2 text-medium-emphasis mt-1">Agora com validações mais claras, métricas e consciência do plano atual.</div>
      </div>

      <ImageUploadDropzone :model-value="form.imageUrls" :max="4" @select-files="handleSelectFiles" @remove="removeImage" />
      ...
      <v-row>
        ...
        <v-col cols="12" md="6"><v-text-field v-model="form.quantity" label="Quantidade" type="number" /></v-col>
      </v-row>
      <v-textarea v-model="form.description" label="Descrição" rows="3" />
      <v-textarea v-model="characteristicsInput" label="Características" hint="Uma por linha ou separadas por vírgula" persistent-hint rows="4" />
      <v-row>
        <v-col cols="12" md="6">
          <v-select v-model="form.categoryId" :items="categories" item-title="name" item-value="id" label="Categoria" />
        </v-col>
        <v-col cols="12" md="6">
          <v-select v-model="form.subcategoryId" :items="filteredSubcategories" item-title="name" item-value="id" label="Subcategoria" />
        </v-col>
      </v-row>
      <v-select v-model="form.status" :items="['active', 'draft']" label="Status" />

      <v-alert v-if="!canSave" type="info" variant="tonal">Preencha nome, descrição, preço e ao menos uma imagem para salvar.</v-alert>
      ...
      <div class="d-flex ga-3 mt-2">
        <v-btn color="primary" :loading="loading || uploading" :disabled="!canSave || (!isEditing && !canCreateProduct)" @click="saveProduct">Salvar produto</v-btn>
