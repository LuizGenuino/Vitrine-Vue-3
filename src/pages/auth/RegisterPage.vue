...
<template>
  <v-card class="glass-panel pa-6 pa-md-8">
    ...
    <div class="text-body-1 text-medium-emphasis mb-6">Crie sua conta e configure sua vitrine em poucos minutos.</div>

    <v-alert v-if="error" type="error" variant="tonal" class="mb-4">{{ error }}</v-alert>
    <v-alert v-if="success" :type="form.password === form.confirmPassword ? 'success' : 'warning'" variant="tonal" class="mb-4">
      {{ success }}
    </v-alert>
    ...
    <v-form @submit.prevent="handleSubmit">
      <div class="d-flex flex-column ga-4">
        ...
        <AppTextField v-model="form.password" label="Senha" type="password" hint="Use uma senha forte para sua conta" />
