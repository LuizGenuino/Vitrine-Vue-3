...
export const categoryService = {
  async listCategories(ownerId: string) {
    const snapshot = await getDocs(query(categoriesCollection, where('ownerId', '==', ownerId), orderBy('order', 'asc')));
    return snapshot.docs.map((item) => ({ id: item.id, ...item.data() })) as Category[];
  },
  async listSubcategories(ownerId: string) {
    const snapshot = await getDocs(query(subcategoriesCollection, where('ownerId', '==', ownerId), orderBy('order', 'asc')));
