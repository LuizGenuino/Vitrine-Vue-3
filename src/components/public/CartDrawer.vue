...
const checkoutLink = computed(() =>
    storefrontStore.settings.channels.whatsappNumber && visibleItems.value.length
        ? buildCartLink(storefrontStore.settings.channels.whatsappNumber, visibleItems.value)
        : '#',
);
...
<template>
    <v-navigation-drawer v-model="uiStore.cartDrawerOpen" location="right" temporary width="420">
        <div class="pa-5 d-flex flex-column" style="height: 100%">
            ...
            <div v-if="visibleItems.length" class="d-flex flex-column ga-3 flex-grow-1"
                style="min-height: 0; overflow: auto">
                <v-card v-for="item in visibleItems" :key="`${item.storeSlug}-${item.productId}`" variant="outlined"
                    class="pa-3">
                    <div class="d-flex ga-3 align-center">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="text-body-2 text-medium-emphasis mt-1">{{ formatCurrency(item.price) }} • {{
                                item.quantity }} unidade(s)</div>
                        </div>
            ...
            <v-card v-else variant="tonal" class="pa-4">
                ...
                <div class="text-body-2 text-medium-emphasis mt-1">Adicione produtos para ver um resumo rápido aqui.
                </div>
            </v-card>
            ...
            <div class="mt-5 pt-4 border-t">
                ...
                <div class="d-flex flex-column ga-3">
                    <v-btn color="primary" size="large" :disabled="!props.storeSlug || !visibleItems.length"
                        @click="goToCart">
                        Abrir carrinho completo
                    </v-btn>
                    <v-btn variant="outlined" size="large"
                        :disabled="!storefrontStore.settings.channels.whatsappNumber || !visibleItems.length"
